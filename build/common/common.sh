...
cd ${FOLDER_NAME}
git add .
git commit -m "Compile-${CODE}-${LUCI_EDITION}-${TARGET_PROFILE}"
...
find . -name 'toml11' -o -name 'mosdns' -o -name 'v2dat' -o -name 'luci-app-passwall' | xargs -i rm -rf {}
...
if [[ ! -f "${PATH1}/resources/apply-patches.sh" ]] && [ -n "$(ls -A "${PATH1}/patches" 2>/dev/null)" ]; then
