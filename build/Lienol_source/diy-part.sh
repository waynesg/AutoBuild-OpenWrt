...
sed -i "s/OpenWrt /AutoBuild Firmware Compiled By @ GWen124 build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ          # 增加个性名字${Author}默认为你的github账号

sed -i 's/PATCHVER:=4.14/PATCHVER:=4.9.294/g' target/linux/x86/Makefile                             # 默认内核为4.14，修改内核为4.19
