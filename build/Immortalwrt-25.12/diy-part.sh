...
PASSWALL2_CONTROLLER="package/waynesg/luci-app-passwall2/luci-app-passwall2/luasrc/controller/passwall2.lua"
if [[ -f "${PASSWALL2_CONTROLLER}" ]]; then
	sed -i 's/_("PassWall 2")/_("Passwall")/g' "${PASSWALL2_CONTROLLER}"
fi
...
#sed -i 's/"Tailscale"/"TailScale虚拟网络"/g' feeds/luci/applications/luci-app-tailscale-community/root/usr/share/luci/menu.d/luci-app-tailscale-community.json

TIME y "调整 VPN 菜单顺序"
sed -i 's/"order": 90,/"order": 10,/' feeds/luci/applications/luci-app-zerotier/root/usr/share/luci/menu.d/luci-app-zerotier.json
#sed -i 's/"order": 90,/"order": 20,/' feeds/luci/applications/luci-app-tailscale-community/root/usr/share/luci/menu.d/luci-app-tailscale-community.json
